  ...
  return (
    <div style={{ minHeight:'100vh', background:COLORS.bg, paddingBottom:100, position:'relative' }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily:SERIF, fontSize:34, color:COLORS.ink, letterSpacing:'-0.02em' }}>My Trips<span style={{fontFamily:'monospace',fontSize:11,color:COLORS.mute,marginLeft:8}}>v286</span></div>
      </div>
      {loading
...
function StopSheet({ open, dayHue, onClose, onSave, cityBias, onRegisterEdit }) {
  ...
  React.useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (dy < 0) {
        // 위로 → 고무줄 (저항 0.25)
        sheetYRef.current = dy * 0.25; setSheetY(dy * 0.25);
      } else {
        // 아래로 → 닫기
    ...
    const onEnd = () => {
      ...
      if (cur > 100) {
        ...
        sheetYRef.current = 0; setSheetY(0); // 고무줄 원위치 or 작은 드래그 복귀
      }
    };
    el.addEventListener('touchstart', onStart, { passive: true });
    el.addEventListener('touchmove', onMove, { passive: false });
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        ...
        <span style={{ fontFamily:SANS, fontSize:14, fontWeight:600, color:COLORS.ink }}>
