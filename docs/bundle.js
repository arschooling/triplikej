...
function TripsScreen({
  ...
  }, "v356"))), loading ? /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'center',
      padding: 60,
      color: COLORS.mute,
...
function PrepScreen({
  ...
  const findPrepTarget = y => {
    ...
    for (let ci = 0; ci < cats.length; ci++) {
      ...
      for (let ii = 0; ii < items.length; ii++) {
        const rect = rects[`${ci}_${ii}`];
        if (!rect) continue;
        if (y >= rect.top && y < rect.bottom) {
          return {
            ci,
            ii: y < rect.top + rect.height / 2 ? ii : ii + 1
          };
        }
      }
      // 카테고리 마지막 항목 아래 빈 공간
      if (items.length > 0) {
        ...
        if (lastRect) {
          ...
          if (y > lastRect.bottom && y < nextTop) return {
            ci,
            ii: items.length
          };
        }
  ...
  const makePrepItemProps = (ci, ii) => ({
    ref: el => {
      prepItemEls.current[`${ci}_${ii}`] = el;
    },
    style: (() => {
      if (!prepDrag) return {};
      const {
      ...
      if (fromCi === toCi && fromCi === ci) {
        if (fromIi < toIi && ii > fromIi && ii <= toIi) shift = -itemH;else if (fromIi > toIi && ii >= toIi && ii < fromIi) shift = itemH;
      } else {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    ref: el => {
      prepItemEls.current[`${ci}_empty`] = el;
    },
    style: {
      padding: '14px 16px',
      fontFamily: SANS,
